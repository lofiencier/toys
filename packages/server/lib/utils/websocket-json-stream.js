...
class WebSocketJSONStream extends Duplex {
  ...
  _send(json, callback) {
    ...
    //   const send = () => {
    //     this.ws.off('open', send)
    //     this.ws.off('close', send)
    //     this._send(json, callback)
    //   }
    //   this.ws.on('open', send)
    //   this.ws.on('close', send)
    console.log('json', json); // } else if (this.ws.readyState === OPEN) {

    this.ws.send(json, callback); // } else {
  ...
  _closeWebSocket(code, reason, callback) {
    switch (this.ws.readyState) {
      case CONNECTING:
        {
          const close = () => {
            this.ws.off('open', close);
            this.ws.off('close', close);

            this._closeWebSocket(code, reason, callback);
          };

          this.ws.on('open', close);
      ...
      case OPEN:
        {
          const closed = () => {
            this.ws.off('close', closed);

            this._closeWebSocket(code, reason, callback);
          };

          this.ws.on('close', closed);
...
exports.default = WebSocketJSONStream;